...
from .io_util import save_output_images, save_colorful_images
...
def train(args, train_loader, model, criterion, optimizer, epoch,
          eval_score=accuracy, print_freq=10):
    batch_time = AverageMeter()
    ...
    for i, (img, target) in enumerate(train_loader):
        ...
        time_left = sec_to_str(batch_time.avg * (len(train_loader) - i))
        ...
        if i % print_freq == 0:
            logger.info('Epoch: [{0}][{1}/{2}]\t'
                        ...
                        'Score {top1.val:.3f} ({top1.avg:.3f})'.format(
                            epoch, i, len(train_loader), time_elapsed, time_left, batch_time=batch_time,
